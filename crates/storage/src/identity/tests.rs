...
#[tokio::test]
async fn no_snapshot_surface_binding_upsert_enriches_ens_v1_binding_manifest_provenance()
-> Result<()> {
    let database = TestDatabase::new().await?;
    let resource_id = Uuid::from_u128(0x2131);
    let surface_binding_id = Uuid::from_u128(0x3131);
    let anchored_observed_at = timestamp(946_684_800);
    let legacy_provenance = json!({
        "adapter": "ens_v1_unwrapped_authority",
        "authority_kind": "registry_only",
        "authority_key": "registry-only:base-mainnet:0x3131",
    });
    let enriched_provenance = json!({
        "adapter": "ens_v1_unwrapped_authority",
        "authority_kind": "registry_only",
        "authority_key": "registry-only:base-mainnet:0x3131",
        "binding_source_family": "basenames_base_registry",
    ...
    upsert_resources(
        ...
        &[resource(
            ...
            None,
            "ens",
            "resource_binding_provenance_enrichment",
            121,
            CanonicalityState::Canonical,
        )],
    )
    ...
    upsert_name_surfaces(
        database.pool(),
        &[name_surface(
            "ens:binding-provenance-enrichment.eth",
            "binding-provenance-enrichment.eth",
            "binding-provenance-enrichment.eth",
            "surface_binding_provenance_enrichment",
            122,
            CanonicalityState::Finalized,
        )],
    )
    ...
    let mut legacy_binding = binding(BindingSeed {
        surface_binding_id,
        logical_name_id: "ens:binding-provenance-enrichment.eth",
        resource_id,
        binding_kind: SurfaceBindingKind::DeclaredRegistryPath,
        active_from: timestamp(1_717_171_700),
        active_to: None,
        source: "legacy_declared_registry_path",
        chain_label: "binding_provenance_enrichment",
        block_number: 123,
        canonicality_state: CanonicalityState::Canonical,
    });
    legacy_binding.provenance = legacy_provenance;
    upsert_surface_bindings_without_snapshots(
        database.pool(),
        std::slice::from_ref(&legacy_binding),
    )
    .await?;
    ...
    let enriched_binding = SurfaceBinding {
        provenance: enriched_provenance.clone(),
        ..legacy_binding
    };
    upsert_surface_bindings_without_snapshots(
        database.pool(),
        std::slice::from_ref(&enriched_binding),
    )
    .await?;

    assert_eq!(
        load_surface_binding(database.pool(), surface_binding_id)
            .await?
            .map(|binding| binding.provenance),
        Some(enriched_provenance)
    );
    assert!(
        load_surface_binding_observed_at(database.pool(), surface_binding_id).await?
            > anchored_observed_at
    ...
    let conflicting_binding = SurfaceBinding {
        provenance: json!({
            "adapter": "ens_v1_unwrapped_authority",
            "authority_kind": "registry_only",
            "authority_key": "registry-only:base-mainnet:0x3131",
            "binding_source_family": "basenames_base_registry",
            "binding_manifest_version": 7,
            "binding_manifest_id": 808,
        }),
        ..enriched_binding
    };
    let error = upsert_surface_bindings_without_snapshots(
        database.pool(),
        std::slice::from_ref(&conflicting_binding),
    )
    .await
    .expect_err("conflicting binding manifest provenance must fail closed");
    assert!(
        error
            .to_string()
            .contains("bulk surface-binding upsert skipped 1 rows"),
        "{error:#}"
    );
...
#[tokio::test]
async fn surface_binding_upsert_enriches_ens_v1_binding_manifest_provenance() -> Result<()> {
    let database = TestDatabase::new().await?;
    let resource_id = Uuid::from_u128(0x2132);
    let surface_binding_id = Uuid::from_u128(0x3132);
    let legacy_provenance = json!({
        "adapter": "ens_v1_unwrapped_authority",
        "authority_kind": "registry_only",
        "authority_key": "registry-only:base-mainnet:0x3132",
    });
    let enriched_provenance = json!({
        "adapter": "ens_v1_unwrapped_authority",
        "authority_kind": "registry_only",
        "authority_key": "registry-only:base-mainnet:0x3132",
        "binding_source_family": "basenames_base_registry",
        "binding_manifest_version": 9,
        "binding_manifest_id": 909,
    });
    ...
    upsert_resources(
        ...
        &[resource(
            ...
            None,
            "ens",
            "resource_binding_row_path_provenance_enrichment",
            124,
            CanonicalityState::Canonical,
        )],
    )
    ...
    upsert_name_surfaces(
        database.pool(),
        &[name_surface(
            "ens:binding-row-path-provenance-enrichment.eth",
            "binding-row-path-provenance-enrichment.eth",
            "binding-row-path-provenance-enrichment.eth",
            "surface_binding_row_path_provenance_enrichment",
            125,
            CanonicalityState::Finalized,
        )],
    )
    ...
    let mut legacy_binding = binding(BindingSeed {
        surface_binding_id,
        logical_name_id: "ens:binding-row-path-provenance-enrichment.eth",
        resource_id,
        binding_kind: SurfaceBindingKind::DeclaredRegistryPath,
        active_from: timestamp(1_717_171_700),
        active_to: None,
        source: "legacy_declared_registry_path",
        chain_label: "binding_row_path_provenance_enrichment",
        block_number: 126,
        canonicality_state: CanonicalityState::Canonical,
    });
    legacy_binding.provenance = legacy_provenance;
    upsert_surface_bindings(database.pool(), std::slice::from_ref(&legacy_binding)).await?;
    ...
    let snapshots =
        upsert_surface_bindings(database.pool(), std::slice::from_ref(&enriched_binding)).await?;

    assert_eq!(snapshots, vec![enriched_binding.clone()]);
    assert_eq!(
        load_surface_binding(database.pool(), surface_binding_id).await?,
        Some(enriched_binding)
    );
