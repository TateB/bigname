...
- the bootstrap declared `claimed_primary_name` contract stays exact-tuple and claim-local: `status` is always present, the admitted claimed-local fields beyond bare status are `claimed_primary_name.raw_claim_name` and exact-tuple declared `claimed_primary_name.provenance`, and `claimed_primary_name.name` remains deferred
...
- `claimed_primary_name.name` remains deferred until a later doc-first contract update freezes a declared normalized claim-identity source that does not collapse into resolver-backed or execution-derived truth
...
- `primary_names_current(address, coin_type, namespace)` is the only admitted claim-side lookup / invalidation anchor for the current bootstrap declared handling and for the verified tuple; persisted claim-local inputs there do not by themselves publish `claimed_primary_name.name`, admit fallback-expanded claim sources, or publish `verified_primary_name` fields beyond the separately frozen verified readback slice
