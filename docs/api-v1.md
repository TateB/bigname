...
- public verified resolution support is narrower than the full declared topology model: in the shipped Phase 7 slice, `namespace=ens` exact-surface direct-path requests are supported first, and the first additive non-direct support class is ENS alias-only paths
...
- the first additive ENS alias-only non-direct support class is the exact-surface class where alias rewriting participates on that same declared topology snapshot, `alias.final_target` is non-`null` with `hops` non-empty, `wildcard.source=null` with `matched_labels=[]`, and all `transport` fields are `null`
- ENS non-direct verified requests outside that alias-only class, including ancestor-selected resolver paths without alias rewriting, wildcard-derived paths, and any transport-assisted path, remain deferred and return `200` with `verified_queries[*].status=unsupported` for every requested selector
...
- the additive alias-only support class and the remaining support-boundary `unsupported` results do not change the mixed route envelope, selector order, query parameters, the shared route-level `coverage` object, or the already-published machine-readable route shape in `docs/api-v1.openapi.json`
...
- the shipped public explain surface follows the same verified-resolution support boundary as the mixed route: in Phase 7, persisted ENS exact-surface direct-path verified answers are in scope first, and the first additive non-direct class is persisted ENS alias-only paths under that same envelope
...
- ENS non-direct paths outside that alias-only class and all `namespace=basenames` verified requests remain outside the shipped public explain surface until a later doc-first contract change broadens verified support; this route does not synthesize trace-shaped `unsupported` responses from declared topology or bootstrap execution scaffolding
...
- for ENS on Ethereum Mainnet, declared `claimed_primary_name` readback is sourced from the exact `primary_names_current(address, coin_type, namespace)` row for the requested tuple; the route does not trigger a fresh reverse-claim lookup while serving that declared status readback
...
- in Phase 7, that reverse-only ENS claim precedence and status-only projection readback do not combine with resolver-backed name data or verification-derived identity to populate richer `claimed_primary_name` fields
...
- the shipped declared `claimed_primary_name` readback is status-only for the exact tuple: `primary_names_current(address, coin_type, namespace)` contributes the declared `ResultStatus` outcome only and does not yet populate `claimed_primary_name.name`, `claimed_primary_name.raw_claim_name`, or claim-local section provenance
...
- `verified_primary_name` is frozen to the shipped mixed-route field boundary `{status, name?, unsupported_reason?, failure_reason?}`; `name`, when present on persisted ENS readback, uses the shared `NameRef` shape, `raw_claim_name` never appears on this execution-derived object, and section-local `verified_primary_name.provenance` remains deferred
...
- `primary_names_current(address, coin_type, namespace)` is the only admitted claim-side lookup / invalidation anchor for that declared status readback and for the verified tuple; tuple presence or claim-local invalidation inputs projected there do not by themselves populate richer `claimed_primary_name` or `verified_primary_name` fields
- `claimed_primary_name.name` and `claimed_primary_name.raw_claim_name` remain deferred to a later additive claimed slice; if later shipped, `claimed_primary_name.name` appears only for `status=success` and `claimed_primary_name.raw_claim_name` appears only for `status=invalid_name`; in the shipped persisted-readback slice, `verified_primary_name.name` appears only for `status=success` or `status=mismatch`, where the route established a concrete normalized name target for verification
- the mixed route continues to use the same already-published bootstrap envelope in `docs/api-v1.openapi.json`; the additive claim-status readback and persisted ENS verified-readback slices change only the documented/readback contents of `claimed_primary_name` and `verified_primary_name` under that envelope and do not widen query parameters, snapshot handling, or route-level coverage
...
- when the admitted declared claim cannot be normalized, `claimed_primary_name` may still return `status=invalid_name`, but the raw claim text itself remains deferred until a later additive claimed slice; `verified_primary_name.status=invalid_name` remains limited to verification-local fields
