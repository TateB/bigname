...
Wire format for the model in [`architecture.md`](architecture.md). Per-route reference in [`api-v1-routes.md`](api-v1-routes.md). Machine-readable contract in [`api-v1.openapi.json`](api-v1.openapi.json).

This file covers conventions, snapshot selection, the response envelope, shared objects, the error model, and versioning.
...
- Semantic identities are strings; opaque internal UUIDs are returned as-is and never derived by clients.
- `namespace` is explicit on canonical name routes; convenience routes infer it but echo it back.
...
| `chain_positions` | Explicit selector — JSON object matching `ChainPositions`, URL-encoded. |
...
Each route documents the subset it honours. Defaults: `consistency=head`, `mode=declared`, no `at` and no `chain_positions` selects the latest stored checkpoint per required chain. On-demand verified execution targets the same selected positions, never a provider's newer head.
...
- Malformed input, duplicate slots, mixed deployment profiles, or a `chain_id` that doesn't match the active profile reject with `invalid_input`.
...
- A coherent selector whose required projection rows aren't built yet returns `stale` — never a raw-fact read.
- Persisted-readback routes return `stale` or `not_found` when matching output is absent. Supported ENS verified resolution is the exception: `GET /v1/resolutions/{namespace}/{name}` and `GET /v1/resolve/{name}` may execute on demand against the selected snapshot, persist the outcome, and return it.
- A current-state row may serve a later selected snapshot only when its stored chain context covers the same required chains and no newer canonical input exists for it through the selected positions; otherwise `stale`.

Cross-chain rules:

- ENS authoritative positions are selected on `ethereum-mainnet`.
...
- Auxiliary chain positions are chosen at the same `consistency` with timestamp at or before the authoritative-chain timestamp.
...
- The ENSv2 `sepolia-dev` profile, when selected, supports declared exact-name profile reads against the admitted `ETHRegistry` and `ETHRegistrar`. It does not enable mainnet, reverse/primary, wrapper authority, migration, Universal Resolver, verified resolution, or execution-explain surfaces.

### The exact-name snapshot

These routes share the same snapshot:
...
- `at`, `chain_positions`, and `consistency` resolve once before any lookup, topology build, explain build, or execution.
...
- A `name_current` or `record_inventory_current` row whose stored position predates the selected snapshot stays eligible only when no newer canonical input exists for its keys through the selected positions and the chain set matches.
...
- `mode=verified|both`: persisted verified output joins only when its stored chain positions match the selected snapshot exactly. If matching output is missing for a supported ENS selector, the route executes against the selected snapshot, persists the outcome, and returns it. Live execution never advances positions mid-request.
...
- Live ENS verified resolution requires an Ethereum RPC provider on the API process. Missing or unable to serve the selected block fails with `409 stale` and a configuration message; declared cache is not a fallback.
- Handlers serve from projections and execution output. Raw facts and adapter-owned normalised events are never read directly.
...
- `mode=declared` populates `declared_state`, sets `verified_state=null`. `mode=verified` does the inverse. `mode=both` populates both.
...
- `meta=none` omits `meta`; collection `page` stays. `meta=summary` includes route-level support, unsupported filters/fields, count metadata, and snapshot summary. `meta=full` adds the full envelope's `coverage`, `chain_positions`, `consistency`, `last_updated`, and route-level `provenance`.
...
| `source_classes_considered` | array |
...
`{ status: "unsupported", unsupported_reason }`. Used when a documented declared subdocument exists but isn't projected. The field stays present.

### `ExactNameControlSummary`

`registrant`, `registry_owner`, `latest_event_kind`. Narrow `declared_state.control` for one resource — not a `ControlVector` dump or a permissions ledger. Keys stay present when supported; values may be `null` when the authority epoch doesn't expose that subject or no retained pointer exists.
...
ENSv1 complete family coverage and resolver-overview support require admission to an ENS Labs PublicResolver-generation profile[^v1-pres-l20]. Retained generic resolver-local events may produce observed cache successes while profile state is `pending`. ENSv1 resolver `NameChanged` text observed via reverse/primary paths is preimage only — it does not make `GET /v1/resolve/{name}` found, doesn't prove primary truth, and doesn't populate records without matching forward-node observations[^v1-namechanged-l10].

Basenames complete family coverage requires a discovered Base resolver to be `L2Resolver`-compatible and admitted as `supported`[^bn-l2resolver-l22]. The ENSv1 profile gate, L1 transport, and offchain gateways don't satisfy this.
...
`surface_head`, `resource_head`. Either may be `null` when the anchor set has no canonical rows. No `both_head` — use the dedicated history route with `scope=both`.
...
| Field | Type / meaning |
|---|---|
| `registry_path` | ordered `NameRef` toward registry authority. Never empty when supported. |
| `subregistry_path` | ordered `NameRef` toward nearest declared subregistry ancestor. Empty when none participates. |
...
`record_key`, `record_family`, `selector_key`, `cacheable`. `record_key` is the round-trip token used in the `records` query parameter. `selector_key` is `null` for scalar families, a string otherwise. When non-null, `record_key = record_family + ":" + selector_key`. Numeric domains (coin types) stay textual.

### `ResolutionRecordGap`

`record_key`, `record_family`, `selector_key`, `gap_reason`. `selector_key=null` covers the scalar family key.
...
May be authoritative for exact lookup while `globally_enumerable=false`. When `topology.resolver_path` ends in the explicit no-resolver hop, inventory is supported with empty selectors and requested cache entries return `not_found` rather than unsupported.
...
`resolver_address`, `text_records`, `known_text_keys`, `avatar`, `content_hash`, `coin_addresses`. `known_text_keys` is declared inventory metadata, not verified enumeration. Value source for `text_records`, `avatar`, `content_hash`, `coin_addresses` follows `mode`. ENSv1 text records are selector-keyed (e.g. `avatar` is `text:avatar`)[^v1-pres-l20]. When `mode=auto|verified|both` has no declared selectors to work from, compact routes may probe the basic app profile set: `addr:60`, `avatar`, `contenthash`, and text keys `description`, `url`, `email`. Fallback text keys that resolve to `not_found` are omitted unless requested explicitly.
...
Per-route semantics live in [`api-v1-routes.md`](api-v1-routes.md). Quick map:

| Route | Purpose |
|---|---|
| `GET /v1/namespaces/{namespace}` | Namespace metadata |
| `GET /v1/manifests/{namespace}` | Active manifest versions and capabilities |
| `GET /v1/names` | Compact name search, exact lookup, address relations |
| `GET /v1/names/{namespace}/{name}` | Exact name lookup (full envelope) |
| `GET /v1/names/{namespace}/{name}/children` | Direct children |
| `GET /v1/names/{namespace}/{name}/records` | Compact records |
| `GET /v1/names/{namespace}/{name}/roles` | Compact role rows for the name's resource |
...
| `GET /v1/history/names/{namespace}/{name}` | Surface or combined name history |
...
| `GET /v1/roles` | Compact role rows |
| `GET /v1/resources/lookup` | `{namespace, name}` → current `resource_id` |
| `GET /v1/resources/{resource_id}/permissions` | Resource-centric permissions |
...
| `GET /v1/resolutions/{namespace}/{name}` | Mixed declared+verified resolution |
| `GET /v1/resolve/{name}` | Namespace-inferred resolution convenience |
| `GET /v1/resolve/{name}/records` | Namespace-inferred records convenience |
| `GET /v1/primary-names/{address}` | Claimed and verified primary name |
| `GET /healthz` | Liveness check (not part of `v1`) |

The running API also serves `GET /openapi.json` and `GET /docs`. They aren't `v1` business routes and don't appear in `docs/api-v1.openapi.json` as path entries.
...
| history routes | `chain_position_desc` |
| `GET /v1/events` | `chain_position_desc` |

Other routes don't honour `cursor` or `page_size`. Surface-first views break ties on `logical_name_id`; resource-grouped address views break on `resource_id`. `page.cursor` echoes the applied cursor or `null` on the first page; `page.next_cursor=null` means no further rows. Cursors are stable under replay for the same chain positions.
...
- Malformed snapshot selectors, unsupported position slots, missing required slots, mixed-profile positions, and `at` with `chain_positions` together return `400 invalid_input`.
- A coherent selector that can't be served from current projections returns `409 stale`.
- A selector whose lineage, canonicality floor, or cross-chain reconciliation can't form one snapshot returns `409 conflict`.
