...
- for `namespace=basenames`, the first `verified_primary_name` support class on this route is exact-tuple persisted readback only for the requested `(address, namespace, coin_type)` tuple; it stays execution-derived under `basenames_execution`, uses the same `request_type=verified_primary_name` and exact tuple request-key identity `{namespace}:{normalized_address}:{coin_type}`, keeps `primary_names_current(address, coin_type, namespace)` as the only claim-side lookup / invalidation anchor, and does not add a dedicated manifest capability flag or change bootstrap route-level coverage because upstream keeps reverse-name writes on the Base ReverseRegistrar while verified resolution enters through the separate Ethereum Mainnet `L1Resolver` (upstream: .refs/basenames/README.md:L22 @ basenames@1809bbc) (upstream: .refs/basenames/README.md:L33 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/ReverseRegistrar.sol:L12 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/ReverseRegistrar.sol:L193 @ basenames@1809bbc) (upstream: .refs/basenames/src/L1/L1Resolver.sol:L13 @ basenames@1809bbc)
...
- tuple presence, tuple absence, a verified mismatch, persisted ENS or Basenames `verified_primary_name` readback, or resolver-backed verification detail does not by itself graduate that bootstrap primary-name coverage summary
