...
- Persisted verified-primary rows are reusable only when the trace/outcome cache identity still matches the requested tuple, requested chain positions, manifest versions, topology boundary, and record boundary. Cache-identity drift is treated as a miss: ENS/60 may continue through the on-demand fallback, supported materialized tuples return the documented verified `not_found`, and unsupported classes remain `unsupported`; active manifest changes are handled by execution invalidation rather than a route-local manifest lookup.
