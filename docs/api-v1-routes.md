...
Per-route reference. Conventions, snapshot selection, the response envelope, shared objects, and the error model are in [`api-v1.md`](api-v1.md).
...
- Identity: `logical_name_id`, `namespace`, `normalized_name`, `canonical_display_name`, `namehash`.
...
- For `namespace=ens` on the ENSv2 `sepolia-dev` profile, exact-name profile reads come from `ens_v2_registry_l1` registry/token/lifecycle/resolver-target events plus `ens_v2_registrar_l1` `.eth` registration and renewal events from the admitted `ETHRegistry` and `ETHRegistrar`[^v2-deploy-ethreg][^v2-deploy-ethrc][^v2-iperm-l34][^v2-iethreg-l32]. Coverage: `status=full`, `exhaustiveness=authoritative`, `source_classes_considered=["ens_v2_registry_l1","ens_v2_registrar_l1"]`, `enumeration_basis=exact_name_profile`. Doesn't widen mainnet, reverse/primary, wrapper authority, migration, Universal Resolver, verified resolution, or execution-explain.
- For `namespace=basenames`, declared truth comes from the Base authority split (`basenames_base_registry`, `basenames_base_registrar`, `basenames_base_resolver`). `basenames_base_primary`, `basenames_l1_compat`, and `basenames_execution` don't widen this route[^bn-readme-l70][^bn-revreg-l12].
...
- `declared_state.history.surface_head` and `resource_head` point at the first canonical rows of `GET /v1/history/names/{namespace}/{name}` under `scope=surface` and `scope=resource`. No `both_head`; use `scope=both` on the dedicated route.
...
The top-level `coverage` object equals the inline `coverage` from `GET /v1/names/{namespace}/{name}` for the same request. For ENSv2 `sepolia-dev`, coverage follows the same exact-name profile rule as that route. No `include` expansions.

## `GET /v1/explain/names/{namespace}/{name}/surface-binding`

Surface-binding explain over the exact-name target.
...
`surface_binding_id` identifies the current `SurfaceBinding` row whose `binding_kind` matches the exact-name answer. No historical binding rows. Reads `surface_bindings_current` plus the existing history truth families. No `include` expansions.

## `GET /v1/explain/names/{namespace}/{name}/authority-control`

Authority/control explain over the exact-name target.
...
Compact app-facing collection: exact lookup, address-owned lists, owner/registrant/effective-controller relations, name search.
...
- `name` is exact lookup by normalized name. With `namespace`, returns zero or one item.
- `prefix`, `contains`, `contains_nocase` are search filters compatible with `namespace`, address relation filters, and pagination. Not availability checks.
...
- For `namespace=basenames`, child surfaces come from the admitted Base authority split only[^bn-readme-l70].
...
- `avatar=true` is a compact alias for the `avatar` text key and may also populate top-level `avatar` from declared cache.
...
Resolves the current `resource_id` for `{namespace, name}` at the exact-name snapshot and returns `RoleRow` items for that resource. If role projection is unavailable, returns empty `data` only when the route can prove no current rows exist; otherwise non-2xx `unsupported` or `409 stale`. `resource_hex` follows the same nullable rule as `GET /v1/resources/lookup`.
...
- `include=role_summary` is additive. Groups current `GET /v1/resources/{resource_id}/permissions` rows by `subject` and keeps `(scope, effective_powers)` pairs. Row-granular grant lineage stays on the dedicated permissions route.
- `subname_count` reuses declared-direct-child semantics from `GET /v1/names/{namespace}/{name}/children`.
- `status` and `expiry` mirror the current `ControlVector.status` and `.expiry` for the item's `resource_id`.
...
- For `namespace=basenames`, address-name membership and relations come from the Base authority split. Reverse-claim and transport state don't add rows or widen relations[^bn-readme-l70].
...
- For ENSv1 wrapper-backed resources, current NameWrapper fuses fold into `effective_powers`. A burned fuse removes any public power that depends on the prohibited operation; rows whose powers are fully masked are omitted. Upstream emits fuses through `NameWrapped` and `FusesSet` and gates wrapper operations on those bits[^v1-iname-l31][^v1-nw-l421].
- A wrapper-backed answer is `full` only when the current fuse modifier was applied. If the projection can't prove current fuse state, the route fails closed rather than returning unmasked powers.
...
- `{namespace, name}` resolves through `GET /v1/resources/lookup` semantics, then reads current effective permission rows.
...
- `provenance` is compact section provenance. Row-granular lineage stays on `GET /v1/resources/{resource_id}/permissions`.

## `GET /v1/resolvers/{chain_id}/{resolver_address}`

Resolver overview. Full envelope.
...
- `aliases` reuses the same `{status, count, items}` envelope but narrows `items` to `binding_kind=resolver_alias_path`. No historical alias enumeration.
- For an enumerable target with no current alias binding, `aliases` returns `{status:"supported", count:0, items:[]}`.
- For ENSv1 PublicResolver-generation targets admitted through the supported gate, `bindings`, `aliases`, and resolver event fan-in summaries return `UnsupportedSummary` with `unsupported_reason="resolver_binding_enumeration_not_projected"` rather than enumerating every name pointing at that shared resolver[^v1-pres-l20]. Exact-name resolver state stays on exact-name and resolution routes.
- A discovered ENSv1 target with `pending` or `unsupported` profile state, or an admitted legacy generation without the requested family, returns `UnsupportedSummary` rather than zero-count latest-PublicResolver summaries.
- For Basenames, a discovered target requires `L2Resolver`-compatible `supported` profile state for the requested family; otherwise `UnsupportedSummary`. The ENSv1 gate, L1 transport, and offchain gateways don't satisfy this[^bn-l2resolver-l22].
...
Compact resolver overview using the same target and `resolver_current` boundary as the full route.
...
- `view=full` delegates to the full route when supported; otherwise reserved.
...
```json
{
  "topology": {
    "registry_path": [
      {"logical_name_id": "ens:alice.eth", "namespace": "ens", "normalized_name": "alice.eth", "canonical_display_name": "alice.eth", "namehash": "0x...", "resource_id": "00000000-...", "binding_kind": "declared_registry_path"}
    ],
    "subregistry_path": [],
    "resolver_path": [
      {"logical_name_id": "ens:alice.eth", "namespace": "ens", "normalized_name": "alice.eth", "canonical_display_name": "alice.eth", "resource_id": "00000000-...", "chain_id": "ethereum-mainnet", "address": "0x...", "latest_event_kind": "ResolverChanged"}
    ],
...
- For ENSv1 and Basenames, a current resolver target alone doesn't claim complete `record_inventory`, `record_cache`, or resolver-overview support. Retained resolver-local events may produce selector-level cache successes; complete family coverage requires resolver-profile admission.
...
- `verified_queries` returns one result per requested selector in request order. Unsupported selector families, unsupported verified path classes, and namespaces without a verified entrypoint return `200` with `verified_queries[*].status=unsupported`. Never silent downgrade to declared cache.
...
|---|---|---|
| ENS | direct | `resolver_path[0]` is the route surface, no wildcard, no alias, no transport |
| ENS | alias-only non-direct | direct + `alias.final_target` non-null with non-empty `hops` |
| ENS | wildcard-derived | `wildcard.source` non-null with matched labels; `resolver_path[0]` matches `wildcard.source`; no alias, no `subregistry_path`, no transport |
| Basenames | exact-surface transport-assisted direct path | `resolver_path[0]` is route surface; no wildcard, alias, or subregistry path; `transport.source=base-mainnet, target=ethereum-mainnet, contract=0xde9049…f31`. CCIP-Read participation is in scope[^bn-l1resolver-l154][^bn-l1resolver-l173]. |

Other ENS classes (non-alias ancestor-selected, linked-subregistry ancestor-selected, transport-assisted, CCIP-participating) return selector-local `unsupported`. Other Basenames classes likewise return `unsupported`.
...
For `mode=verified|both`, top-level `provenance` summarises the request-scoped trace; `verified_queries[*]` may carry narrower per-selector provenance.
...
- Inferred ENS in `mode=verified|both` shares the canonical cache-or-live-execute behaviour and the same fail-closed RPC-provider requirement.
...
- `verified_state.execution.execution_trace_id == provenance.execution_trace_id`. `verified_queries[*].provenance` stays under that same trace id.
...
- For ENS, the admitted claim source is `ens_v1_reverse_l1` reverse intake at `0xa58E81fe…fc7Cb`[^v1-revreg-l15]. The route doesn't trigger fresh reverse lookups while serving the declared response. Missing or unsupported claims don't fall back to registry, resolver, or other claim-setting surfaces.
...
- For Basenames, the admitted claim family is `basenames_base_primary` at `0x79ea96…f9e282`[^bn-revreg-l12]. Claim intake only — does not replace the Base registry/registrar/resolver families for declared truth on exact-name, address-name, or children reads.
- `claimed_primary_name.raw_claim_name` may appear only when `status=invalid_name` for the exact requested tuple, copied verbatim from `primary_names_current.raw_claim_name`. Blank or whitespace-only raw claims become `not_found`; `invalid_name` is for nonblank claims that fail normalisation.
- `claimed_primary_name.provenance` is exact-tuple declared provenance from the requested row. It strips any verified-primary lookup/invalidation hook material and omits `execution_trace_id`.
- `verified_primary_name` shape: `{status, name?, unsupported_reason?, failure_reason?, provenance?}`. `name` uses `NameRef` and appears only for `success` or `mismatch`. `raw_claim_name` never appears here.
- `verified_primary_name.provenance` (when present) is `{execution_trace_id, manifest_versions}`. `execution_trace_id` equals top-level `provenance.execution_trace_id`.
- `verified_primary_name` is authoritative only on `status=success`. `status=mismatch` means the claim normalises and the verified target resolves for the requested `coin_type` but doesn't equal the requested `{address}`.
...
| Class | `coverage` |
|---|---|
| ENS mainnet supported tuple | `status=partial`, `exhaustiveness=non_enumerable`, `source_classes_considered=["ens_v1_reverse_l1","ens_execution"]`, `enumeration_basis=primary_name_lookup` |
| Basenames mainnet supported tuple | `status=partial`, `exhaustiveness=non_enumerable`, `source_classes_considered=["basenames_base_primary","basenames_execution"]`, `enumeration_basis=primary_name_lookup` |
| Out of class | `status=unsupported`, `exhaustiveness=not_applicable`, `source_classes_considered=[]`, `unsupported_reason="primary-name exact-tuple persisted readback is not supported for the requested tuple"` |

Out-of-class verified objects use `verified_primary_name.status=unsupported`. Tuple presence, absence, mismatch, or resolver-backed verification detail doesn't change these states. Class membership chooses route-level coverage; result-object `status` describes the tuple answer inside that class.

## App-facing examples

```text
# Owned-names dashboard
...
GET /v1/roles?resource_id=00000000-…&page_size=50
