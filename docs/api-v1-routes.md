...
- Pagination for name-qualified reads uses `account_resource_scope_asc` over the combined resource and root-resource rows. The cursor remains the same `(account, resource_id, scope)` keyset tuple.
