...
The ENSv2 `sepolia-dev` exact-name profile remains shadow-only in Phase 5. Admitting the upstream `ETHRegistry` and `ETHRegistrar` deployments, and consuming registry resource/token events, does not make the public exact-name profile supported; a later doc-first manifest promotion must change `exact_name_profile` from `shadow` to `supported` before API coverage can graduate (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistry.json:L2 @ ens_v2@554c309) (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistrar.json:L2 @ ens_v2@554c309) (upstream: .refs/ens_v2/contracts/src/registry/interfaces/IPermissionedRegistry.sol:L34 @ ens_v2@554c309) (upstream: .refs/ens_v2/contracts/src/registry/interfaces/IRegistryEvents.sol:L69 @ ens_v2@554c309).
...
- within that ENSv2 Phase 5 profile, `ens_v2_registrar_l1` owns the current `exact_name_profile=shadow` capability and no other `sepolia-dev` family implies exact-name profile support; `rollout_status=active`, registry admission, registrar admission, and resolver-family admission are intake readiness, not public exact-name profile graduation (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistry.json:L2 @ ens_v2@554c309) (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistrar.json:L2 @ ens_v2@554c309)
...
- for Basenames on the shipped mainnet profile, that exact-tuple persisted verified-primary support class stays execution-derived under `basenames_execution` rather than `basenames_base_primary`: upstream keeps reverse-name writes on the Base ReverseRegistrar while verified resolution enters through the separate Ethereum Mainnet `L1Resolver`, so this freeze does not add a dedicated manifest capability flag or widen bootstrap route-level coverage (upstream: .refs/basenames/README.md:L22 @ basenames@1809bbc) (upstream: .refs/basenames/README.md:L33 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/ReverseRegistrar.sol:L12 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/ReverseRegistrar.sol:L193 @ basenames@1809bbc) (upstream: .refs/basenames/src/L1/L1Resolver.sol:L13 @ basenames@1809bbc)
...
- for `namespace=ens` under the selected ENSv2 `sepolia-dev` profile, Phase 5 exact-name profile support remains shadow-only: the public exact-name profile coverage is `status=unsupported`, `exhaustiveness=not_applicable`, and `unsupported_reason="ensv2 sepolia-dev exact-name profile is shadow-only"` until `exact_name_profile` is promoted to `supported`; shadow registry/registrar intake may support later projection work, but it must not be hidden as supported public coverage (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistry.json:L2 @ ens_v2@554c309) (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistrar.json:L2 @ ens_v2@554c309)
...
- ENSv1 Phase 4 NameWrapper and PublicResolver admission does not by itself make wrapper-backed history, wrapper migration history, resolver overview, exact-name profile, primary-name, or verified-resolution coverage supported; unsupported or unprojected sections remain explicit until a later doc-first coverage update
- the ENSv2 `sepolia-dev` exact-name profile is not a supported source class while its manifest capability is `shadow`; its exact-name profile coverage must stay `unsupported` rather than `full`, `partial`, or `observed_only` (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistry.json:L2 @ ens_v2@554c309) (upstream: .refs/ens_v2/contracts/deployments/sepolia-dev/ETHRegistrar.json:L2 @ ens_v2@554c309)
...
- the shipped primary-name route remains bootstrap-only for coverage: route presence, tuple lookup, or persisted ENS or Basenames `verified_primary_name` readback does not by itself graduate primary-name coverage beyond `status=unsupported` and `exhaustiveness=not_applicable`
