...
2026-07-03 Basenames Base registry-only derivation-change repair record: commit 12bcea0 intentionally moved registry-only ENSv1 authority resource derivation from a labelhash-scoped key to the current node/namehash-scoped key. Its checked-in repair record admitted only the Basenames `AuthorityTransferred` parity case; the six-kind Basenames widening was not anticipated there. The deployment corpus that predates that derivation change contains `basenames_base_registry` normalized events across `base-mainnet` blocks 17,577,060 through 46,945,560 that conflict with current replay: `PermissionChanged` 1,440,626; `ResolverChanged` 780,482; `AuthorityEpochChanged` 623,763; `SurfaceBound` 623,762; `SurfaceUnbound` 41,284; `AuthorityTransferred` 6,925; 664,470 resources total. The ratified method is the sanctioned Rust repair machinery, not an ad-hoc SQL migration: on re-observation it verifies the same event identity, logical name, chain, source family, event kind, canonical resource anchors, and legacy/current labelhash relationship, then rewrites only the re-derived `resource_id` and derivation-affected state fields listed above. The repair appends normalized-event projection changes and directly enqueues stale and repaired resource keys for `permissions_current` on `AuthorityTransferred`/`PermissionChanged` and `record_inventory_current` on `ResolverChanged`. After the supervised repair run, operators must drain replay/apply or rebuild `name_current`, `address_names_current`, `permissions_current`, and `record_inventory_current`. `name_current` consumes the normalized-event changes for all six kinds by logical name, `address_names_current` depends on the repaired authority/name state after `name_current`, and the two resource-keyed projections consume the directly enqueued stale and repaired resource keys. `children_current`, `primary_names_current`, `resolver_current`, and execution caches are not direct rebuild targets for this six-kind repair unless a separate run changes their inputs.
