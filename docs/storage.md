...
- `surface_bindings.provenance` is otherwise identity-bearing and exact-match immutable. The only compatibility enrichment is for legacy `ens_v1_unwrapped_authority` rows written before binding manifest provenance was persisted: storage may accept and rewrite an incoming row when the stored and incoming provenance objects match after removing `binding_source_family`, `binding_manifest_version`, and `binding_manifest_id`, and every binding field already present on the stored row matches the incoming value. Missing or conflicting binding provenance fails closed for all other adapters and shapes.
