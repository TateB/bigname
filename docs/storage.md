...
2026-07-03 Basenames Base registry-only derivation-change repair record: commit 12bcea0 intentionally moved registry-only ENSv1 authority resource derivation from a labelhash-scoped key to the current node/namehash-scoped key. Its checked-in repair record admitted only the Basenames `AuthorityTransferred` parity case; widening beyond that was not anticipated there. The maintainer-ratified option (i) method is the sanctioned Rust repair machinery, not an ad-hoc SQL migration. Observation rows keep the in-place repair path because their event identities do not include the changed authority key: `PermissionChanged` 1,440,626; `AuthorityTransferred` 6,925; and the observation-scoped portion of the 780,482 `ResolverChanged` rows. Boundary rows use canonicality supersession because their identities include the changed authority key: `AuthorityEpochChanged` 623,763; `SurfaceBound` 623,762; `SurfaceUnbound` 41,284; and the boundary-scoped portion of `ResolverChanged` where `transaction_hash IS NULL`, `log_index IS NULL`, and `after_state.source_event = AuthorityEpochChanged`. The exact `ResolverChanged` split must be recorded from the supervised-run preflight query below; the checked-in repository does not contain the deployment corpus needed to recompute that split.
