...
- `phase8-basenames-execution-rollout-rule`: `basenames_execution` keeps `verified_resolution=shadow` until that exact supported class ships on both the mixed and execution-explain routes; only then may the manifest capability graduate to `supported`, and that promotion does not move transport ownership away from `basenames_l1_compat`, widen route-level primary-name coverage beyond the separate exact-tuple persisted-readback contract, or add a dedicated primary-name manifest flag (upstream: .refs/basenames/README.md:L22 @ basenames@1809bbc) (upstream: .refs/basenames/README.md:L69 @ basenames@1809bbc) (upstream: .refs/basenames/src/L1/L1Resolver.sol:L13 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/ReverseRegistrar.sol:L12 @ basenames@1809bbc)
...
- `phase10-live-manifest-drift-audit-job`: live manifest-drift and proxy-alert production is frozen as worker-owned observation-only operational state over admitted manifests, stored code-hash facts, stored proxy / implementation edges, and derived watch-plan state; it computes operational audit output without writing adapter-owned `normalized_events` or a new alert table unless a later doc-first worker-owned persistence family exists, preserves the proxy `contract_instance_id` while implementation churn remains an edge observation, and does not mutate manifest truth, discovery admission, capability flags, watch-plan inputs, projections, public APIs, or consumer-replacement meaning.
