...
- `phase10-manifest-drift-alert-inspection-cli`: read-only manifest-drift and proxy-alert inspection is frozen as operational JSON over existing stored alert observations, including their source manifest, source family, chain, contract-instance references, proxy / implementation edge references, expected and observed code-hash material, derived watched-target metadata, timestamps, lifecycle status, and nullable remediation metadata; it does not perform fresh chain comparison, create observations, mutate alert lifecycle state, admit contracts, change capability flags, rewrite discovery edges, mutate watch-plan inputs, write projections, expose public APIs, or claim consumer replacement.
