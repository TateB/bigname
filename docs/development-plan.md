...
- `phase7-primary-name-claim-status-contract-clarification`: the shipped declared `claimed_primary_name` readback on `GET /v1/primary-names/{address}` is frozen to the exact `primary_names_current(address, coin_type, namespace)` tuple as a status-only declared readback; the route does not trigger fresh reverse-claim lookup while serving that readback, the admitted ENS claim state remains reverse-only through `ens_v1_reverse_l1`, richer claimed fields stay deferred, and route-level primary-name coverage remains bootstrap `unsupported`
