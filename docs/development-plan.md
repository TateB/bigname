...
- `phase7-verified-resolution-support-boundary-clarification`: the shipped mixed `GET /v1/resolutions/{namespace}/{name}` contract and shipped `GET /v1/explain/resolutions/{namespace}/{name}/execution` envelope stay stable, and public verified support is frozen to ENS exact-surface direct-path reads plus the first additive ENS alias-only non-direct class, using the same declared-topology support check as the route contract; wildcard-derived paths, transport-assisted paths, non-alias ENS non-direct paths, and all Basenames verified reads remain explicit unsupported until a later doc-first contract update broadens the slice after Base authority plus L1 compatibility transport are wired
- `phase7-ens-verified-resolution-alias-path-contract-clarification`: the first additive ENS non-direct verified-resolution support class is alias-only on both the mixed and explain routes: it admits persisted ENS exact-surface alias-path answers under the existing envelopes when alias rewriting participates and wildcard plus transport do not, while keeping query parameters, wildcard support, transport support, Basenames support, and machine-readable publication in `docs/api-v1.openapi.json` unchanged
...
- `phase7-primary-name-claim-status-contract-clarification`: the shipped declared `claimed_primary_name` surface on `GET /v1/primary-names/{address}` is frozen to the exact `primary_names_current(address, coin_type, namespace)` tuple as a bootstrap status-shaped declared contract; the route does not trigger fresh reverse-claim lookup while serving that bootstrap readback, the admitted ENS claim state remains reverse-only through `ens_v1_reverse_l1`, tuple-present declared reads stay limited to that status-shaped object, and route-level primary-name coverage remains bootstrap `unsupported`
- `phase7-primary-name-claim-status-readback-bootstrap`: the shipped route now reads back the exact tuple's declared `claimed_primary_name.status` from `primary_names_current` in `mode=declared|both`, keeps the public declared field boundary status-only, and still defers `raw_claim_name`, normalized claimed identity, claim-local provenance, and any coverage graduation
- `phase7-primary-name-claim-local-fields-contract-clarification`: the next honest additive claimed-local field beyond that bootstrap status shape is `raw_claim_name` only, sourced verbatim from `primary_names_current.raw_claim_name` for the exact tuple and only for `claim_status=invalid_name`; normalized claimed-name identity and public claim-local provenance remain deferred so the route does not smuggle resolver-backed or execution-derived truth into `claimed_primary_name`, and this clarification does not change the current machine-readable publication or bootstrap coverage
